/**
 * Verificacion de la migracion: compara el CSS de HEAD con el actual, regla a regla.
 *
...
const readHead = (f) => { try { return execSync(`git show HEAD:${f}`, { maxBuffer: 1 << 28 }).toString(); } catch { return ''; } };

/** Devuelve un mapa selector-normalizado -> cuerpo-normalizado de todas las reglas. */
function rules(src, out = new Map(), prefix = '') {
  const noC = src.replace(/\/\*[\s\S]*?\*\//g, ' ');
  let i = 0, bs = 0;
  while (i < noC.length) {
    // Un `;` a nivel cero cierra una at-rule sin bloque (`@use '...' as *;`). Sin esto
    // se quedaba pegada al selector siguiente, que pasaba por at-rule y se contaba mal.
    ...
    if (noC[i] === '{') {
      ...
      if (sel.startsWith('@')) rules(body, out, prefix + sel + ' | ');
      else {
        const decls = body.split(';').map((x) => x.trim().replace(/\s+/g, ' ')).filter((x) => x && !x.includes('{')).sort();
        for (const s of sel.split(',').map((x) => x.trim()).filter(Boolean)) out.set(prefix + s, decls.join(';'));
      }
      i = bs = j;
      continue;
    }
...
// Una regla desaparecida solo esta bien si ninguna de sus clases se usa ya en el markup.
...
console.log(`reglas en HEAD: ${before.size}   ahora: ${after.size}`);
...
if (changed.length) {
  console.log('\nCUERPO CAMBIADO (no deberia haber ninguna en un movimiento mecanico):');
  for (const s of changed.slice(0, 25)) console.log('   ' + s.slice(0, 140));
}
process.exit(suspicious.length || changed.length ? 1 : 0);
