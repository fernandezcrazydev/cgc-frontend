...
function createComponent(lobbies: LobbyResponse[]) {
  ...
  const fixture = TestBed.createComponent(Tablon);
...
describe('Tablon', () => {
  ...
  it('múltiples convocatorias a menos de media hora se muestran en directo y se excluyen de lo que viene', () => {
    const sala1 = lobby('s1', {
      confirmedSlotId: 'slot-1',
      slots: [slot('slot-1', '2026-09-05T22:00:00Z', 10, 0)],
    });
    const sala2 = lobby('s2', {
      confirmedSlotId: 'slot-2',
      slots: [slot('slot-2', '2026-09-05T22:05:00Z', 10, 2)],
    });
    const lejana = lobby('s3', {
      confirmedSlotId: 'slot-3',
      slots: [slot('slot-3', '2026-09-06T22:00:00Z')],
    });
    const { component } = createComponent([sala1, sala2, lejana]);

    expect(component.liveLobbies().map((l) => l.id)).toEqual(['s1', 's2']);
    expect(component.liveLobby()?.id).toBe('s1');
    expect(component.scheduled().map((e) => e.lobby.id)).toEqual(['s3']);
  });
