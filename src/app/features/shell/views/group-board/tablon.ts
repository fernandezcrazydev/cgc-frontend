...
/**
 * El **Tablón** del grupo (`Roadmap.md` §5.5.6): a la izquierda lo que se juega ahora,
 * a la derecha lo que viene.
 *
 * Se llamaba «Partidas», y el nombre mentía: aquí no hay ninguna partida. Con el
 * vocabulario de `FlujoJuego.md` §2 una **partida** es una custom ya jugada —eso está en
 * Historial—, mientras que esta pantalla enseña **convocatorias** y **salas**. Un tablón
 * anuncia lo que pasa ahora y lo que viene, que es exactamente sus dos columnas.
 *
...
export class Tablon {
  ...
  /**
   * Las convocatorias que ya se juegan o están a punto: confirmadas y a menos de media
   * hora de su hora. Si hay varias, van todas a la columna de salas en directo ordenadas
  ...
  /**
   * Primera sala en directo (retrocompatibilidad con tests y referencias de sala única).
   */
  readonly liveLobby = computed<LobbyResponse | null>(() => {
    return this.liveLobbies()[0] ?? null;
  });
  ...
  slotOf(lb: LobbyResponse): LobbySlotResponse | null {
    return confirmedSlotOf(lb);
  }

  kickoffOf(lb: LobbyResponse): string {
    const slot = confirmedSlotOf(lb);
    return slot ? formatKickoff(slot.startsAt) : '';
  }
   ...
   * primera libre.
   */
  canJoinLive(lb: LobbyResponse): boolean {
    const slot = confirmedSlotOf(lb);
    if (!slot) return false;
    if (slot.starters.length >= lb.capacity) return false;
    return standingOf(lb, this.myUserId()).kind === 'out';
  }

  joiningLive(lb: LobbyResponse): boolean {
    const slot = confirmedSlotOf(lb);
